    ...
    <div class="card p-4 shadow-sm" style="background:#f5f0e4; border-radius:16px;">
        ...
        <form method="POST" action="{{ route('seller.products.update', $product['id']) }}" enctype="multipart/form-data">
            ...
            <div class="row g-4 mt-3">
                ...
                <div class="col-md-6">
                    <div class="p-3" style="background:white; border-radius:10px; border:1px solid #ddd;">
                        ...
                        <input type="text" name="category" class="form-control mb-3" value="{{ $product['category'] }}">
                        ...
                        <div class="row g-2">
                            <div class="col"><input type="number" name="length" class="form-control" value="{{ $product['length'] }}"></div>
                            <div class="col"><input type="number" name="breadth" class="form-control" value="{{ $product['breadth'] }}"></div>
                            <div class="col"><input type="number" name="width" class="form-control" value="{{ $product['width'] }}"></div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-3" style="background:white; border-radius:10px; border:1px solid #ddd;">
                        ...
                        <div class="d-flex gap-3">
                            <button type="button" class="btn btn-outline-secondary 
                                {{ $product['price_type']=='fixed' ? 'active' : '' }}">Fixed Price</button>

                            <button type="button" class="btn btn-warning
                                {{ $product['price_type']=='negotiation' ? 'active' : '' }}">Negotiation</button>
                        </div>
                    </div>
                </div>
            ...
            <div class="text-end mt-4">
                <button class="btn btn-warning px-4">Update Product</button>
            </div>
